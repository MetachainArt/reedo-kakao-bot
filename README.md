...
- **대화 맥락 이해** — 최근 대화를 보고 상황에 맞는 답장
- **친구 이름 표시** — KakaoTalk2.db 복호화로 실제 이름 조회
- **오픈채팅방 지원** — 그룹/오픈채팅방 이름 자동 조회
- **24시간 자동 실행** — systemd 서비스로 재부팅해도 자동 시작
...
# 5. (선택) systemd 서비스로 24시간 자동 실행
bash setup-systemd.sh
```

---

## systemd로 24시간 자동 실행 (VPS)

터미널을 닫아도 봇이 계속 동작하고, 서버 재부팅 시 자동 시작됩니다.

### 자동 설정

```bash
bash ~/reedo-kakao-bot/setup-systemd.sh
```

### 수동 설정

```bash
# 1. sudo 비밀번호 없이 docker/chmod 실행 허용
sudo bash -c 'echo "YOUR_USER ALL=(ALL) NOPASSWD: /usr/bin/docker cp *, /usr/bin/chmod *" > /etc/sudoers.d/reedo-daemon'

# 2. 서비스 파일 복사 및 등록
sudo cp ~/reedo-kakao-bot/reedo-daemon.service /etc/systemd/system/
...
```bash
# 상태 확인
sudo systemctl status reedo-daemon

# 로그 보기
tail -f ~/reedo-daemon.log

# 재시작
sudo systemctl restart reedo-daemon

# 중지
sudo systemctl stop reedo-daemon
```
...
| `model` | AI 모델 | "openai-codex/gpt-5.4" | 다른 모델로 변경 가능 |
...
├── reedo-daemon.service         # systemd 서비스 파일 (VPS 24시간 실행)
│
...
├── setup-systemd.sh             # systemd 서비스 등록 (VPS)
├── stop.sh                      # 종료 스크립트
│
...
├── ADB (localhost:5555)
├── kakaodecrypt (DB 복호화)
│   ├── KakaoTalk.db → chat_logs_dec (메시지)
│   └── KakaoTalk2.db → friends_dec (친구 이름), open_link (채팅방 이름)
├── OpenClaw (AI 에이전트)
├── systemd (reedo-daemon.service — 24시간 자동 실행)
...
- `kakaodecrypt.py` — DB 복호화 (`chat_logs` → `chat_logs_dec`, `friends` → `friends_dec` 테이블 생성)

**두 개의 DB를 복호화합니다:**
- `KakaoTalk.db` — 메시지, 채팅방 정보
- `KakaoTalk2.db` — 친구 이름, 오픈채팅방 이름
...
봇 성격은 SOUL.md에서 관리하므로 daemon 프롬프트는 최소한의 컨텍스트만 전달합니다.
...
| AI가 항상 같은 대답 | openclaw 모델 설정 오류 | `~/.openclaw/openclaw.json`에서 agent 모델 확인 |
...
| 터미널 닫으면 봇 죽음 | 프로세스가 터미널에 종속 | systemd 서비스 등록 (`setup-systemd.sh`) |
| systemd에서 sudo 비밀번호 요구 | 대화형 sudo 불가 | `/etc/sudoers.d/`에 NOPASSWD 설정 |
| 친구 이름이 숫자로 표시 | KakaoTalk2.db 미복호화 | daemon이 자동으로 KakaoTalk2.db도 복호화 |
...
**A: 약 30-35초.** DB 복사(2초) + 복호화(2초) + AI 생성(25-30초) + 전송(1초). AI 모델 응답 시간이 대부분입니다.
...
**A: 네.** systemd 서비스로 등록하면 터미널을 닫아도, 서버를 재부팅해도 자동으로 시작됩니다.
...
### Q: 다른 채팅방으로 자동 전환되나요?
**A: 현재는 열려있는 채팅방에서만 전송합니다.** 카카오톡 앱에서 해당 채팅방을 미리 열어두세요.
